...
const EMPTY_TYPE = { name: '', limit: 1, handSlots: 0, excludes: [] }
...
export default function EquipmentTypeManager() {
  ...
  function startEdit(type) {
    setEditId(type.id)
    setForm({ name: type.name, limit: type.limit, handSlots: type.handSlots ?? 0, excludes: type.excludes ?? [] })
  }
  ...
  async function handleSave(e) {
    ...
    try {
      if (editId) {
        await updateDoc(doc(db, 'equipmentTypes', editId), { name: form.name.trim(), limit: Number(form.limit), handSlots: Number(form.handSlots ?? 0), excludes: form.excludes })
      } else {
        await addDoc(collection(db, 'equipmentTypes'), { name: form.name.trim(), limit: Number(form.limit), handSlots: Number(form.handSlots ?? 0), excludes: form.excludes, createdAt: serverTimestamp() })
      }
      cancelEdit()
      load()
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '340px 1fr', gap: '1rem' }}>
        ...
        <Card title={editId ? 'EDIT TYPE' : 'ADD TYPE'}>
          <form onSubmit={handleSave}>
            ...
            <F label="HAND SLOTS (0 = nie zajmuje rąk, 1 = jednoręczny, 2 = dwuręczny)">
              <select value={form.handSlots ?? 0} onChange={e => setForm(f => ({...f, handSlots: Number(e.target.value)}))} style={inp}>
                <option value={0}>0 — nie zajmuje rąk</option>
                <option value={1}>1 — jednoręczny</option>
                <option value={2}>2 — dwuręczny</option>
              </select>
            </F>

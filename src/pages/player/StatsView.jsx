...
function StatRow({ label, statKey, baseValue = 0, breakdown }) {
  ...
  const finalRaw = bd?.final ?? baseValue          // precyzyjny float
  const final    = Math.floor(finalRaw)            // zawsze w dół
  ...
  function tooltipContent() {
    ...
    // Mode mnożniki — pokaż z modeSources lub fallback z modeMultiplier
    ...
    // Zawsze pokaż precyzyjną wartość w tooltip
...
function PDRModal({ character, race, onClose }) {
          ...
          {STAT_DISPLAY.map(stat => {
            ...
            return (
              <div key={stat} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.5rem 0', borderBottom: '1px solid var(--color-bo-border)' }}>
                <span style={{ flex: 1, fontSize: '0.7rem', letterSpacing: '0.1em', color: 'var(--color-bo-text-dim)' }}>{getStatName(stat, race).toUpperCase()}</span>
...
export default function StatsView() {
  ...
  const race = c.race ?? ''
  const s    = c.stats ?? {}
  const nazoTree = { stat: 'nazo', nodes: c?.nazoNodes ?? [], edges: [] }
  const allTrees  = { ...globalTrees, nazo: nazoTree }
  const { effective, breakdown } = calcEffectiveStats(c, items, globalTrees)
    ...
    <PageShell title="STATISTICS" subtitle={`${c.alias ? c.alias + ' · ' : ''}${race || '—'}`}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', alignItems: 'start' }}>
        ...
        <Card title="COMBAT STATISTICS">
          ...
          <StatRow label={c.nazoUnlocked ? (c.nazoName ?? '???').toUpperCase() : '???'} statKey="nazo" baseValue={s.nazo} breakdown={breakdown} />
        </Card>
        <PassiveEffectsPanel character={c} allTrees={allTrees} effectiveStats={effective} />
        </div>
        ...
        <div>
          ...
          <Card title="HEALTH & REIATSU POINTS" style={{ marginBottom: '1rem' }}>
            {[
              { label: 'HEALTH POINTS', val: calcHP(c, effective), formula: c.hpFormula || DEFAULT_HP_FORMULA, color: '#c04040' },
              { label: 'REIATSU POINTS', val: calcRP(c, effective), formula: c.rpFormula || DEFAULT_RP_FORMULA, color: '#3060c0' },
              ...
              <div key={label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.5rem 0', borderBottom: '1px solid var(--color-bo-border)' }}>
                <div>
                  ...
                  <div style={{ fontSize: '0.5rem', color: 'var(--color-bo-muted)', marginTop: '1px' }}>{formula}</div>
                </div>
                <span style={{ fontFamily: 'var(--font-display)', fontSize: '1.3rem', color, fontWeight: 700 }}>{val}</span>
              </div>
            ))}
          </Card>

          {/* Mode'y — gracz może je toggle'ować */}
          {(c.modes ?? []).length > 0 && (
            <Card title="MODES" style={{ marginBottom: '1rem' }}>
              {(c.modes ?? []).map(mode => (
                <div key={mode.id} style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', padding: '0.45rem 0', borderBottom: '1px solid var(--color-bo-border)' }}>
                  {/* Toggle */}
                  <button
                    onClick={async () => {
                      const updated = (c.modes ?? []).map(m => m.id === mode.id ? { ...m, active: !m.active } : m)
                      await updateDoc(doc(db, 'characters', c.id), { modes: updated })
                    }}
                    style={{ flexShrink: 0, width: 36, height: 20, borderRadius: 10, background: mode.active ? '#60c080' : 'var(--color-bo-muted)', border: 'none', cursor: 'pointer', position: 'relative', transition: 'background 0.2s' }}
                  >
                    <span style={{ position: 'absolute', top: 3, left: mode.active ? 19 : 3, width: 14, height: 14, borderRadius: '50%', background: 'white', transition: 'left 0.2s', display: 'block' }} />
                  </button>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: '0.72rem', color: mode.active ? '#60c080' : 'var(--color-bo-text-dim)', fontWeight: mode.active ? 600 : 400 }}>{mode.name}</div>
                    ...
                    {(mode.multipliers ?? []).length > 0 && (
                      <div style={{ display: 'flex', gap: '3px', flexWrap: 'wrap', marginTop: '3px' }}>
                        {mode.multipliers.map((m, i) => (
                          <span key={i} style={{ fontSize: '0.52rem', color: mode.active ? '#60c080' : 'var(--color-bo-muted)', background: mode.active ? 'rgba(96,192,128,0.08)' : 'none', border: `1px solid ${mode.active ? 'rgba(96,192,128,0.25)' : 'var(--color-bo-border)'}`, padding: '1px 4px' }}>
                            {m.stat} ×{m.factor}
                          </span>
                        ))}
                      </div>
          ...
          <Card title="PERSONNEL INFO">
            ...
            {c.age      && <InfoRow label="AGE"     value={`${c.age}`} />}
          </Card>
        </div>
      </div>
      {showPDR && <PDRModal character={c} race={race} onClose={() => setShowPDR(false)} />}
    </PageShell>
  )
}

...
export default function TechniquesView() {
  ...
  useEffect(() => {
    if (!character?.id) return
    const unsub = onSnapshot(collection(db, 'characters', character.id, 'items'),
  ...
  const { effective: effectiveStats } = calcEffectiveStats(character ?? {}, items, globalTrees)
      ...
      {techniques.length === 0 ? (
        ...
        <>
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map(t => <TechCard key={t.id} tech={t} characterStats={effectiveStats} />)
          )}
        </>
      )}
    </PageShell>

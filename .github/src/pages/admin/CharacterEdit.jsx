...
export default function CharacterEdit() {
  ...
  const [form,      setForm]      = useState(null)     // edytowalne pola
  const [charLive,  setCharLive]  = useState(null)     // live snapshot (dla multipliers)
  ...
  return (
    <PageShell title={'EDIT: ' + (form.identifier ?? '')} subtitle={(form.firstName ?? '') + ' ' + (form.lastName ?? '')}>
      ...
      <Card title="DOSSIER" style={{ marginTop: '1rem' }}>
        ...
        {[['appearance','APPEARANCE'],['personality','PERSONALITY'],['history','HISTORY']].map(([k, lbl]) => (
          <F key={k} label={lbl}>
